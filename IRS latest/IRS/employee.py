...
class employeeClass:
    def __init__(self,root):
        ...
        btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="red",fg="white",cursor="hand2").place(x=620,y=305,width=110,height=28)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="black",fg="white",cursor="hand2").place(x=740,y=305,width=110,height=28)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":  
                ...
                row=cur.fetchone()
                if row!=None:
                      messagebox.showerror("Error","This is Employee id already assigned, try different",parent=self.root)
                else:
