...
class BillClass:
    def __init__(self, root):
        ...
        self.root.title("Inventory Record System")
        self.root.config(bg="white")
        self.cart_list=[]
        # ==title==
        self.icon_title = PhotoImage(file="IRS latest\IRS\images\images\logo1.png")
        ...
        #====================Footer============
        footer=Label(self.root,text="IMS-INVENTORY MANAGEMENT SYSTEM | Developed by Heth,Maitry\n",font=("times new roman",11),bg="#4d636d",fg="white",bd=0,cursor="hand2").pack(side=BOTTOM,fill=X)
    ...
    def generate_bill(self):
        if self.var_cname.get()=='' or self.var_contact.get()=='':
            ...
            fp.close()
            messagebox.showinfo('Saved',"Bill has been generatef / Saved in Backend",parent=self.root)
...
{str('='*47)}\n
    ...
    def update_date_time(self):
        ...
        self.lbl_clock.config(text=f"Welcome to Invetory Record System\t\t Date: {str(date_)}\t\t Time: {str(time_)}")
