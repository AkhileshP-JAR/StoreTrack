...
class productClass:
    def __init__(self, root):
        ...
        lbl_catagory = Label(product_frame, text="CATAGORY", font=(
            "goudy old style", 18)).place(x=30, y=60)
        lbl_supplier = Label(product_frame, text="SUPPLIER", font=(
            "goudy old style", 18)).place(x=30, y=110)
        lbl_name = Label(product_frame, text="NAME", font=(
            "goudy old style", 18)).place(x=30, y=160)
        lbl_price = Label(product_frame, text="PRICE", font=(
            "goudy old style", 18)).place(x=30, y=210)
        lbl_quantity = Label(product_frame, text="QUANTITY", font=(
            "goudy old style", 18)).place(x=30, y=260)
        lbl_status = Label(product_frame, text="STATUS", font=(
            "goudy old style", 18)).place(x=30, y=310)
        ...
        txt_name = Entry(product_frame, textvariable=self.var_name, font=(
            "goudy old style", 15), bg='lightblue').place(x=180, y=160, width=200)
        txt_price = Entry(product_frame, textvariable=self.var_price, font=(
            "goudy old style", 15), bg='lightblue').place(x=180, y=210, width=200)
        txt_qty = Entry(product_frame, textvariable=self.var_qty, font=(
            "goudy old style", 15), bg='lightblue').place(x=180, y=260, width=200)
        ...
        btn_add = Button(product_frame, text="Save", command=self.add, font=(
            "goudy old style", 15), bg="blue", fg="white", cursor="hand2").place(x=10, y=400, width=100, height=40)
        btn_update = Button(product_frame, text="Update", command=self.update, font=(
            "goudy old style", 15), bg="red", fg="white", cursor="hand2").place(x=120, y=400, width=100, height=40)
        btn_delete = Button(product_frame, text="Delete", command=self.delete, font=(
            "goudy old style", 15), bg="black", fg="white", cursor="hand2").place(x=230, y=400, width=100, height=40)
        btn_clear = Button(product_frame, text="Clear", command=self.clear, font=(
            "goudy old style", 15), bg="green", fg="white", cursor="hand2").place(x=340, y=400, width=100, height=40)
        ...
        btn_search=Button(SearchFrame,text="Search",command=self.search,font=("goudy old style",15),bg="#0f4d7d",fg="white",cursor="hand2").place(x=430,y=8,width=150,height=30)
        ...
        self.product_tavle=ttk.Treeview(p_frame,columns=("pid","Category","Supplier","name","price","qty","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.product_tavle.xview)
        scrolly.config(command=self.product_tavle.yview)

        self.product_tavle.heading("pid",text="PRODUCT ID")
        self.product_tavle.heading("Category",text="CATEGORY")
        self.product_tavle.heading("Supplier",text="SUPPLIER")
        self.product_tavle.heading("name",text="NAME")
        self.product_tavle.heading("price",text="PRICE")
        self.product_tavle.heading("qty",text="QUANTITY")
        self.product_tavle.heading("status",text="STATUS")


        self.product_tavle["show"]="headings"

        self.product_tavle.column("pid",width=90)
        self.product_tavle.column("Category",width=90)
        self.product_tavle.column("Supplier",width=90)
        self.product_tavle.column("name",width=90)
        self.product_tavle.column("price",width=90)
        self.product_tavle.column("qty",width=90)
        self.product_tavle.column("status",width=90)

        self.product_tavle.pack(fill=BOTH,expand=1)
        self.product_tavle.bind("<ButtonRelease-1>",self.get_data)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                messagebox.showerror(
                    "Error", "ALL feilds must be required", parent=self.root)
            else:
    ...
    def show(self):
        ...
        try:
            ...
            rows = cur.fetchall()
            self.product_tavle.delete(*self.product_tavle.get_children())
            for row in rows:
                self.product_tavle.insert('', END, values=row)
    ...
    def get_data(self, ev):
        f = self.product_tavle.focus()
        content = (self.product_tavle.item(f))
        row = content['values']
        # print(row)
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get() == "":
                messagebox.showerror(
                    "Error", "Product id must be required", parent=self.root)
            else:
            ...
            if self.var_pid.get() == "":
                ...
                if row == None:
                    ...
                    op = messagebox.askyesno(
                        "Confirm", "Do yo really want to delete?", parent=self.root)
                    if op == True:
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get() == "Select":
                ...
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.product_tavle.delete(
                        *self.product_tavle.get_children())
                    for row in rows:
                        self.product_tavle.insert('', END, values=row)
                else:
                    messagebox.showerror(
